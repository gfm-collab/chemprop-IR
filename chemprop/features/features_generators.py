...
def custom_features_generator(mol: Molecule) -> np.ndarray:
    ...
    mol = Chem.MolFromSmiles(mol) if type(mol) == str else mol  File "<stdin>", line 1, in <module>
...
@register_features_generator('BOB')
def custom_features_generator(mol: Molecule) -> np.ndarray:
#    # If you want to use the SMILES string
#    smiles = Chem.MolToSmiles(mol, isomericSmiles=True) if type(mol) != str else mol
    ...
    for bond in mol.GetBonds():
        bond_atom1_sym = bond.GetBeginAtom().GetSymbol()
        bond_atom2_sym = bond.GetEndAtom().GetSymbol()
        bond_type = [bond_atom1_sym] + [bond_atom2_sym]
        if bond_type==['C','H'] or bond_type==['H','C']:
            features += np.array([1,0,0])
        if bond_type==['O','H'] or bond_type==['H','O']:
            features += np.array([0,1,0])
        if bond_type==['O','C'] or bond_type==['C','O']:
            features += np.array([0,0,1])
